...
boolean t0end = true, t0done = true, s0end = true, s0done = true, p0end = true, p0done = true, drumTimer = false;
Toggle toggle1, toggle2, toggle3, toggle4, toggle5;
float burner1Temp = 50, burner2Temp = 50, burner3Temp = 50, burner4Temp = 50;
int count = 0, overall, nums;
...
void setup() {
  ...
  backgroundmusic = new Gain(ac, 1, 1);
  g.addInput(p0);
  g.addInput(p1);
  g.addInput(p2);
  g.addInput(p3);
  ...
  g.addInput(s3r);
  g.addInput(s4r);
  backgroundmusic.addInput(bg0);
  backgroundmusic.addInput(bg1);
  backgroundmusic.addInput(bg2);
  backgroundmusic.addInput(bg3);
  backgroundmusic.addInput(gc);
...
void mainsystem() {
  ...
  if (timer && end && count == 0 && multidone) {
    end = false;
    timer = false;
    multidone = false;
    count++;
  ...
  } else if (timer && end && count == 1&& multidone) {
  ...
  } else if (timer && end && count == 2&& multidone) {
  ...
  } else if (timer && end && count == 3&& multidone) {
    end = false;
    timer = false;
    multidone = false;
    count++;  
    if (Knob4.getValue() != 0.0) {
      if (burner4Temp == b4.getValue()) { // no temperature change
        if (burner4Temp < 131){burner4met(0,1);}
        else if (131 <= b4.getValue() && b4.getValue() < 212){burner4met(0,2);} 
        else if (212 <= b4.getValue() && b4.getValue() < 325) {burner4met(0,3);}
        else {burner4met(0,4);}
